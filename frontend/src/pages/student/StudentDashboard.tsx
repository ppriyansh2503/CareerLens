...
export const StudentDashboard: React.FC = () => {
  ...
  const highestTier: 'GOLD' | 'SILVER' | 'BRONZE' | 'NONE' = 
    profile.certificates?.some(c => c.badge_tier === 'GOLD') ? 'GOLD' :
    profile.certificates?.some(c => c.badge_tier === 'SILVER') ? 'SILVER' :
    profile.certificates?.some(c => c.badge_tier === 'BRONZE') ? 'BRONZE' : 'NONE';
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <TrustBadge tier={highestTier} size="sm" />
              </div>
              <p className="text-sm text-slate-300 font-medium">
                {profile.headline || 'Student & Engineering Aspirant'}
              </p>
              <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-slate-400">
                <span className="flex items-center gap-1">
                  <GraduationCap size={14} className="text-indigo-400" />
                  {profile.department || 'Computer Science'} ({profile.graduation_year || 2026})
                </span>
                <span>•</span>
                <span>CGPA: <strong className="text-slate-200">{profile.cgpa || 0.0}</strong></span>
                <span>•</span>
                <span className="text-slate-300">{profile.college_name || 'Autonomous Institution'}</span>
              </div>
            </div>
          </div>

          {/* Placement Readiness Gauge & Fast CTAs */}
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-4 bg-slate-800/80 px-6 py-4 rounded-2xl border border-slate-700">
              <div>
                ...
                <div className="text-xs font-semibold text-emerald-400 flex items-center gap-1 mt-0.5">
                  <TrendingUp size={13} />
                  <span>{profile.placement_readiness_score >= 70 ? 'Ready to Place' : 'Building Skills'}</span>
                </div>
              </div>
              <div className="text-3xl sm:text-4xl font-black bg-gradient-to-r from-emerald-400 to-indigo-400 bg-clip-text text-transparent">
                {profile.placement_readiness_score || 0}%
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <Link
                ...
                className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-semibold text-xs transition-all shadow-lg shadow-amber-500/20"
              >
                <Award size={15} />
              ...
              <Link
                to="/student/resume"
                className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium text-xs border border-slate-700 transition-all"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {profile.certificates.length === 0 ? (
            <div className="p-8 rounded-2xl bg-slate-800/30 border border-dashed border-slate-700/80 text-center space-y-3">
              <div className="w-12 h-12 rounded-xl bg-amber-500/10 text-amber-400 flex items-center justify-center mx-auto">
                <ShieldCheck size={24} />
              </div>
              <h3 className="text-sm font-semibold text-white">No Verified Certificates Yet</h3>
              <p className="text-xs text-slate-400 max-w-md mx-auto">
                Upload your course, cloud, or technical certificates. Our 4-tier verification engine uses OCR, QR decoding, and tamper analysis to grant verified badges.
              </p>
              <Link
                to="/student/certificates"
              ...
              >
                <Award size={14} />
                <span>Upload & Verify Your First Certificate</span>
              </Link>
            </div>
          ) : (
